...
describe('cli: task', () => {
  ...
  test('start 登记 + done 归位 (默认 --dir = 进程 cwd)', async () => {
    const s = await run(['task', 'start', 'T1', '--note', '做事']);
    assert.equal(s.code, 0, s.stderr);
    const d = await run(['task', 'done', 'T1']);
    assert.equal(d.code, 0, d.stderr);
    const t = await run(['todo']);
    assert.ok(t.stdout.includes('T1'));
  });

  test('缺 action/id 报用法且退出码非 0', async () => {
    const r = await run(['task']);
    assert.notEqual(r.code, 0);
    assert.ok(/task <start\|done\|blocked\|note>/.test(r.stderr), r.stderr);
  });

  test('未知 action 报错', async () => {
    const r = await run(['task', 'frobnicate', 'T1']);
    assert.notEqual(r.code, 0);
    assert.ok(r.stderr.includes('task <start'), r.stderr);
  });

  test('blocked / note 断点实时落盘', async () => {
    await run(['task', 'start', 'TB', '--note', 'x']);
    const n = await run(['task', 'note', 'TB', '--note', '改到 L40']);
    assert.equal(n.code, 0, n.stderr);
    assert.ok(n.stdout.includes('断点'), n.stdout);
    const b = await run(['task', 'blocked', 'TB', '--note', '端口占用']);
  ...
  test('start 幂等: 同 id 不重复登记', async () => {
    await run(['task', 'start', 'TIDEM', '--note', 'v1']);
    await run(['task', 'start', 'TIDEM', '--note', 'v2']);
    const t = await run(['todo']);
