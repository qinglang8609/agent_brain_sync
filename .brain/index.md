...
## Sources
- [[2026-09-10-opencode-插件-abs-ts-的导出形状]] — opencode 插件 abs.ts 的导出形状确认: @opencode-ai/plugin 要求 PluginModule={id?,server:Plugin}，且 server 是 ({client…
- [[2026-09-10-oc-inject-channel-判定实验结论]] — OC-INJECT-CHANNEL 判定实验结论: promptAsync 对 idle(终结态) session 确实能唤醒跑新 turn, 不是"只入队不重启". 真机证据(plugin-enabled…
- [[2026-09-10-主动推类-hook-回-decision-blo]] — 主动推类 hook(回 decision:block/注入新 turn)必须自带死循环防护: 注入会触发新一轮, 新一轮结束又 fire 同一个 hook。两道守卫缺一不可 —— ①宿主防重入字段(CC 的…
- [[2026-09-10-守卫-guard-的工具白名单必须按宿主实际用法]] — 守卫(guard)的工具白名单必须按宿主实际用法穷举, 漏一个就把功能静默关掉。opencode 侧 wroteFiles 守卫抄 pi 时漏了 bash, 而该宿主大量修改走 bash(heredoc/sed), 纯…
- [[2026-09-10-排查-某行为没发生-先证明上游-gate-满足]] — 排查"某行为没发生"先证明上游 gate 满足, 再怀疑下游通道。顺序错了会白花一轮: 本次零 nudge 先怀疑注入通道(promptAsync 语义), 实际是上游 wroteFiles gate 没过。复现法:…
- [[2026-09-10-类型签名-返回码推不出运行时语义-prompt]] — 类型签名/返回码推不出运行时语义: promptAsync 返回 204 void 被推断为"fire-and-forget 不启动 turn", 实测 2 秒内真产生新 assistant turn…
- [[2026-09-10-宿主-mcp-注册路径必须写-包的稳定安装位置]] — 宿主 MCP 注册路径必须写"包的稳定安装位置", 不能写 ABS_DIR: ABS_DIR = install.js 自己住哪 —— 从仓库跑 abs install 就把仓库路径烧进宿主配置, 移包/卸全局即失效…
...
- [[2026-09-10-try-catch-吞异常会把-写错的代码-变成]] — try/catch 吞异常会把「写错的代码」变成「永远不触发的检查」——本次 lint 的 DONE-PILED-UP 永不报警, 根因是 brainPath(vault,'todo.md')(vault 已是…
- [[2026-09-10-引用新符号后必须真跑一条命令-node-ch]] — 引用新符号后必须真跑一条命令：node --check 只查语法，查不出"未导入的标识符"，跑到那行才炸 ReferenceError。本次一轮内栽两次：store.js 用了 BRAIN_DIR 却只导入了…
- [[2026-09-12-摘要的文字质量由写入口决定-不由写摘要的模型决定]] — 摘要的文字质量由写入口决定，不由写摘要的模型决定：abs 的 log/index 摘要被读起来'抽象'，第一嫌疑不是 agent 表述能力，而是存盘时就被切了——store.js 的 .slice(0,100)…
- [[2026-09-12-散文截断是-摘要读起来抽象-的隐形根因-且它自我]] — 散文截断是「摘要读起来抽象」的隐形根因，且它自我掩盖：硬切后的残句看着像模型表述能力差，于是所有人去调 prompt，没人查写入侧…
## Syntheses
## Sessions
