...
- [x] ENTITY-LINK-USER @fanchao — 作者标记 @name → 「双链」wiki 链接格式; 用户独立实体页 entities/<user>.md (含技术栈/特点, config set user 或首次写操作时创建); extractAuthor 兼容旧 @name 与新「双链」写法; index.md 不塞作者, 改 query/load 输出附带页 author; lint 加「人名占位」有页存在校验 【落地】 (完成 2026-09-12)
