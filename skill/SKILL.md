...
### 排查 Bug：先假设，再复现，用一手数据定位

**核心**：Bug 多数由新增/修改引起，**不凭空猜，用数据说话**；但“能复现”不等于“已定位”，
定位到出错位置 ≠ 找到根因。
...
#### 第 1 步：复现 —— 先让 Bug 稳定出现

没有稳定复现，后面全是猜。**拿不到真实报错就停下来要，不要自己造一个。**

- 复现的必要条件？哪个输入/操作/环境触发？
- 100% 还是偶发？**偶发先找触发模式**（特定数据/时机/顺序），别用单个样本下结论。
- 日志里有没有现成的错误栈？**错误信息 + 行号是最便宜的定位入口。**

**给 Bug 起个名字**（一句话 + 触发条件）。卡住时回去核对是否还在同一个 Bug 上。

#### 第 2 步：定位改动范围 —— Bug 不会凭空出现
...
| 技术栈 | 打印方式 | 日志位置 |
|---|---|---|
| PHP (ThinkPHP) | `\think\Log::error($data)` / `trace($data)` | `runtime/log/` |
| PHP (Laravel) | `Log::info($data)` / `logger($data)` | `storage/logs/` |
| Vue / JS | `console.log(data)` | 浏览器控制台 |
| Node.js | `console.log(data)` / `logger.debug(data)` | 终端 / 日志文件 |
| SQL | `\think\Db::getLastSql()` 或开 SQL 日志 | runtime/log 或控制台 |

**二分法打印**：在可疑链路上隔几层插一个日志点，先看哪段有数据、哪段没数据，范围砍半。
比一次打满所有层快得多。

**技巧**：每个日志点带唯一前缀（`[BUG-01]`），grep 一次看到全链路顺序；
怀疑并发/时序就打时间戳与请求 ID。

#### 第 4 步：顺藤摸瓜 —— 按调用链逐层查

```
用户操作 → 路由 → 门面/中间件 → 控制器 → 服务层 → 模型 → 数据库
```

每层打印关键数据，**找到数据从正确变为错误的那一层，bug 就在那一层**。

#### 第 5 步：找根因 —— 不只修症状

改对症状 = 换一批数据又复发。修复前回答：
- 为什么数据在这层变错了？**逻辑错误**（判断写反/字段取错）/ **类型错误**
  （null/数组/对象混用）/ **数据本身脏**（上游写入时就错）？
- 根因若在上游（写入方/历史数据），这里 patch 只是挡一下 → 去修上游或在入口统一兜底。

**修一层，不改所有调用点**：多个地方调同一函数时，只在出问题那个调用点打补丁，
其他调用点照样坏。**在共享函数里修一次，是所有调用点的最小修复。**

#### 第 6 步：验证 —— 逐行打印确认，别凭空验证

**先拿第 1 步的复现条件重跑**，确认错误消失，再逐层打印确认每步符合预期。三层都要过：
...
| 陷阱 | 正确做法 |
|---|---|
| 凭经验猜 bug 位置 | 先复现 + 看日志/打印，用数据定位 |
| 只看代码不运行 | 打印实际运行时的数据 |
| 一次改很多地方再测 | 改一处、验证一处 |
| 修症状不修根因 | 追问“为什么这层数据变了” |
| 只验证出错路径 | 重跑复现 + 回归正常路径 + 边界输入 |
| 忘记删调试代码 | 确认后清理所有 `console.log` / `\Log::error` |
| 只在本地验证 | 确认服务器代码已同步部署、数据一致 |
| 排查卡住还硬扛 | 回第 1 步核对 Bug 描述；必要时换数据/换触发角度 |
| 偶发当成必然 | 偶发先找触发模式，别用单样本下结论 |

#### 卡住时（>20 分钟无进展，不硬扛）

1. **Bug 描述还准吗？** 新观察是否改变了问题定义。
2. **复现还稳定吗？** 换个触发样本还出现吗。
3. **有没有漏看的日志？** grep 全量（不只最近的），可能早期就报过错。
4. **改动范围查全了吗？** 只看 HEAD~3 可能漏了分支合并、配置、部署差异。
5. **要不要问人？** 最近谁改的、意图是什么，一句话可能点醒。
6. **向上游看一层**：数据/配置来源方是否也变了。
...
> 与上面同一根因：模糊化是逃避判断。要么给结论 + 依据，要么明确索取证据。
