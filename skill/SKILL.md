...
> **先挂上 `bug-hunter` skill**（若该宿主没装，就按本节第 0 步自行排查）：
> 完整排查流程（复现 → 定位改动 → 打印 → 顺藤摸瓜 → 根因 → 验证，含打印/调用链细节）
> 以 `bug-hunter` 为准，本节只保留**判断层**约束 —— 尤其是它没有的那一环：
> **方向错了怎么办**。两份不要重复维护。
...
#### 第 1～6 步：完整流程见 `bug-hunter` skill

**本 skill 不自带完整排查流程**（避免两份要同步）。复现 / 定位改动 / 打印 / 调用链 /
根因 / 验证的细节，以 `bug-hunter` 为准，**先挂它**（该宿主没装就按常识走那六步）。

只需记住三个判断层要点（`bug-hunter` 里没有的）：

1. **拿不到一手错误就先要，别自己造** —— 那条 `app.init` 造假正是从跳过复现开始的。
2. **「能复现」≠「已定位」** —— 定位到出错位置也不等于找到根因（换批数据就复发）。
3. **改在共享函数里，不是每个调用点** —— 多个调用点打补丁 = 漏的照样坏。

> **卡住超过 20 分钟不硬扛**：回第 0 步核对假设列表与反证条件 —— 是不是从一开始
> 方向就错了（而不是"再挖深一点"）。
